...
function Set-PSDWizardDefault{
    $script:Xaml.SelectNodes("//*[@Name]") | ? { $_.Name -like "TS_*" } | % {
        ...
        if($_.Name -eq "TS_Roles"){
            $value = $control.Text
            $control.Items.Add("Marketing")
            $control.Items.Add("Accounting")
            $control.Items.Add("Sales")
            $control.SelectedItem = "Marketing"
        }
    }
}
...
Export-ModuleMember -function Get-PSDWizard
