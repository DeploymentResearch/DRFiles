# Script to extract the Windows Server 2019 Standard index from a Windows Server 2019 media.
# Update line 5 - 9 to match your environment
# Please note that editions in evaluation media are named differently, for example: Windows Server 2019 Standard Evaluation (Desktop Experience)

# General parameteers
$ISO = "C:\iso\Windows Server 2019.iso" # Path to Windows Server 2019 media
$WIMPath = "C:\Ref" # Target folder for extracted WIM file containing Windows Server 2019 Standard only
$WIMFile = "$WIMPath\REFWS2019-001.wim" # Exported WIM File
$Edition = "Windows Server 2019 Standard (Desktop Experience)" # Edition to export

# Goal is to have a single index WIM File, so checking if target WIM File exist (if WIM exist, a new index will be appended)
If (Test-path $WIMFile){
    Write-Warning "WIM File: $WimFile does already exist"
    Write-Warning "Rename or delete the file, then try again. Aborting..."
    Break 
}

# ISO Validation
If (-not (Test-path $ISO)){
    Write-Warning "ISO File: $ISO does not exist, aborting..."
    Break 
}

# Mount ISO
Mount-DiskImage -ImagePath $ISO | Out-Null
$ISOImage = Get-DiskImage -ImagePath $ISO | Get-Volume
$ISODrive = [string]$ISOImage.DriveLetter+":"

# Source WIM validation
$SourceWIMFile = "$ISODrive\sources\install.wim"
If (-not (Get-WindowsImage -ImagePath $SourceWIMFile | Where-Object {$_.ImageName -ilike "*$($Edition)"})){
    Write-Warning "WIM Edition: $Edition does not exist in WIM: $SourceWIMFile, aborting..."
    Break
}

# Export WIM
If (!(Test-path $WIMPath)){ New-Item -Path $WIMPath -ItemType Directory -Force | Out-Null } # Create folder if needed
Export-WindowsImage -SourceImagePath $SourceWIMFile -SourceName $Edition -DestinationImagePath $WIMFile

# Dismount ISO
Dismount-DiskImage -ImagePath $ISO | Out-Null

