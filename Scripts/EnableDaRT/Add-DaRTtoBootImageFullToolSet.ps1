<#
.SYNOPSIS
    Adds the full DaRT toolset to a ConfigMgr boot image.
.DESCRIPTION
    Same as the standard DaRT injection but includes the complete diagnostic toolset rather
    than only the remote connection component.
.LINK
...
if (!(Test-Path -Path "$SampleFiles\UnattendFullToolset.xml")) {Write-Warning "Could not find Unattend.xml, aborting...";Break}
...
Copy-Item $SampleFiles\UnattendFullToolset.xml $MountPath\Unattend.xml
